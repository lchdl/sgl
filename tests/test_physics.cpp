...
struct {
  ...
  OpenGL::Font font;
  sgl::EyeParams eye;
  sgl::Model wedge, box, teapot, cone;
} gl;

struct {
  const double dt = 0.016;
  const int substeps = 8;
  Physics::RigidBody wedges[2], boxes[2], teapots[2], cones[16];
...
void init_render_and_physics() {
  ...
  phys.teapots[0].build(1, hull, 1.0, hull.center_of_mass(), 1.0, &gl.teapot);
  phys.teapots[0].setPos(Vec3(-4.5, 6, 0));
  phys.teapots[0].setName("teapot0");
  phys.teapots[1].build(2, hull, 1.0, hull.center_of_mass(), 1.0, &gl.teapot);
  ...
  phys.boxes[0].build(3, hull, 1.0, hull.center_of_mass(), 1.0, &gl.box);
  phys.boxes[0].setPos(Vec3(-4.5, 4.5, 0));
  phys.boxes[0].setName("box0");
  phys.boxes[1].build(4, hull, 1.0, hull.center_of_mass(), 1.0, &gl.box);
  ...
  phys.wedges[0].build(5, hull, 1.0, hull.center_of_mass(), 1.0, &gl.wedge);
  phys.wedges[0].setPos(Vec3(-1.5, 0, 0));
  phys.wedges[0].setStatic();
  phys.wedges[0].setName("wedge0");
  phys.wedges[1].build(5, hull, 1.0, hull.center_of_mass(), 1.0, &gl.wedge);
  ...
  for (int i = 0; i < 8; i++) {
    phys.cones[i].build(10 + i, hull, 0.5, hull.center_of_mass(), 1.0, &gl.cone);
  ...
  for (int i = 8; i < 16; i++) {
    phys.cones[i].build(20 + i, hull, 0.5, hull.center_of_mass(), 1.0, &gl.cone);
