...
void init_render() {
  ...
  gl.fbuf_render.setup_attachment(&gl.color_attachment, 0);
...
Mat4x4 shadow_pass(double T) {
  ...
  gl.fbuf_depth.bind();
  {
    ...
    const double light_range = 3.0, near_dist = 0.1, far_dist = 10.0;

    Mat4x4 light_view = sgl::get_view_matrix(light_pos, Vec3(0, 0, 0), Vec3(0, 1, 0));
    Mat4x4 light_proj = sgl::get_orthographic_matrix(near_dist, far_dist, -light_range, +light_range, +light_range, -light_range);
    light_transform = light_proj * light_view;

    gl.shader_shadow.use();
    gl.shader_shadow.set_uniform_matrix_4fv("LightTransform", 1, GL_TRUE, &light_transform);
    render_scene(gl.shader_shadow, T);
  }
  gl.fbuf_depth.unbind();
/*
  gl.depth_attachment.to_device(DeviceType_CPU);
  auto tex = gl.depth_attachment;
  tex.to_format(PixelFormat_RGBA8888).save_png("depth.png");
  gl.depth_attachment.to_device(DeviceType_GPU);
*/
...
double render_frame(double T) {
  ...
  {
    Mat4x4 light_transform;
    light_transform = shadow_pass(T);
    main_pass(light_transform, T);
  }
